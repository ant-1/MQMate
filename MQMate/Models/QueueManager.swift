...
// MARK: - Queue Model (Forward Declaration)

/// Queue model representing an IBM MQ queue
/// This is a placeholder that will be replaced by the full Queue model in subtask-3-4
public struct Queue: Identifiable, Sendable, Equatable, Hashable {
    ...
    /// Depth as a percentage of max depth
    ...
    /// Sample queues for previews
    public static let samples: [Queue] = [
        Queue(name: "DEV.QUEUE.1", queueType: .local, depth: 42, maxDepth: 5000),
        Queue(name: "DEV.QUEUE.2", queueType: .local, depth: 0, maxDepth: 5000),
        Queue(name: "DEV.QUEUE.3", queueType: .local, depth: 4500, maxDepth: 5000),
        Queue(name: "DEV.ALIAS.1", queueType: .alias, depth: 0, maxDepth: 0),
        Queue(name: "DEV.REMOTE.1", queueType: .remote, depth: 0, maxDepth: 0)
    ]
